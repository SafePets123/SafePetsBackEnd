...
module.exports={
    async create(req, res){
        try {
            ...
            const result = await knex('denunciante').where({ email });
            if (result.length === 1){
                return res.status(400).send({ erro: 'Email já cadastrado'});
            }
            await knex('clientes')
                .insert({ nome, email, password });
                return res.status(201).send( { nome, email, password});
        } catch (error) {
    ...
    async searchUsersAll(req, res ){
       // try {
            console.log('requisição de lista de usuarios');
        
            const result = await knex('denunciante');
    ...
    async searchUsers( req, res){
        try {
            ...
            console.log(result); //busca de erro
